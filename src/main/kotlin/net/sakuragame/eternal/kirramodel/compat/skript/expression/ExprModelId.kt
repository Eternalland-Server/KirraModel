...
class ExprModelId : SimpleExpression<String>() {
    ...
    override fun init(exprs: Array<Expression<*>>, matchedPattern: Int, isDelayed: Kleenean, parseResult: SkriptParser.ParseResult): Boolean {
        if (!parser.isCurrentEvent(ModelInteractEvent::class.java)) {
            Skript.error("Cannot use 'model-id' outside of ModelInteractEvent", ErrorQuality.SEMANTIC_ERROR)
            return false
        }
        return true
    }
    ...
    override fun get(e: Event?): Array<String>? {
        ...
        return arrayOf(interactEvent.model.id)
    }
}